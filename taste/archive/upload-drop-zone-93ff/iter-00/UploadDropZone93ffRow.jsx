...
export default function UploadDropZone93ffRow({ file, exiting, onRemove, onRetry }) {
  const percent = Math.round(file.progress * 100);
  const develop = Math.min(1, file.progress / 0.45);
  const showPercent =
    ...
    >
      <div className="udz-row-clip">
        <div className="udz-row-body flex items-center gap-3 px-4 py-3">
          <UploadDropZone93ffThumb file={file} develop={develop} />
